...
class ContractController(BaseController):
    ...
    def paginated_contracts_displayed(self, title: str, filter_type: str, client_email_address=None):
        ...
        """
        contract_objects = self.define_data_to_display(filter, client_email_address)
        ...
        buttons_label = self.define_page_buttons_nedded(contracts_label, filter)
    ...
    def create_all_contracts_paginated_buttons_signal(self, paginated_view, buttons, contract_objects):
        ...
        urwid.connect_signal(buttons["other_buttons"][1], "click", lambda button: paginated_view.next_page())
        urwid.connect_signal(
            buttons["other_buttons"][2], "click", lambda button: self.paginated_contracts_displayed("Home > Contracts > My contracts", "current_user")
        )
    ...
    def define_page_buttons_nedded(self, contracts_label, filter_type):
        ...
        """
        if filter_type == "all":
            return {"users_label": contracts_label, "buttons": ["Previous", "Next", "My contracts"]}
